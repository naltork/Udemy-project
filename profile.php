...
// fetch() returns false when no row matches, so fall back to an empty array
// to keep the field lookups below from warning about a bool offset.
if($userres === false){ $userres = array(); }

$userid = isset($userres['id']) ? $userres['id'] : 0;
...
// A user_permission row only exists once the user saves permissions.php, so
// users who never visited that page have no row at all.
if($permres === false){ $permres = array(); }

// A field is shown only when it has a value AND the user allowed sharing it.
// Missing permission rows mean "not shared", which keeps profiles private by
// default until the owner opts in on permissions.php.
...
<div id="<?php echo $nav_logged_in ? 'page-wrapper' : 'guest-wrapper'; ?>">
<div class="container">
	<div class="row">
		<div class="col-lg-6 col-sm-6 col-md-offset-3">
            ...
            <div class="card hovercard">
                ...
                <?php if(can_show($userres, $permres, 'profilepic', 'show_pic')){ ?>
                <div class="avatar">
                    <img alt="" src="<?php echo $base_url . $userres['profilepic']; ?>">
                </div>
                <?php } ?>
                <div class="info">
                    <div class="title">
                        <?php if(can_show($userres, $permres, 'fname', 'show_fname')){ echo $userres['fname']; } ?> <?php if(can_show($userres, $permres, 'lname', 'show_lname')){ echo $userres['lname']; } ?>
                    </div>
                    <div class="desc"><?php if(can_show($userres, $permres, 'gender', 'show_gender')){ echo $userres['gender']; } ?></div>
                    <div class="desc"><?php if(can_show($userres, $permres, 'age', 'show_age')){ echo $userres['age']; } ?></div>
                    <div class="desc"><?php if(can_show($userres, $permres, 'bio', 'show_bio')){ echo $userres['bio']; } ?></div>
                    <div class="desc"> <?php if(can_show($userres, $permres, 'mobile', 'show_mobile')){ 
                        echo '<i class="fa fa-phone"></i> ';
                        echo $userres['mobile']; } ?></div>
                    <div class="desc"> <?php if(can_show($userres, $permres, 'email', 'show_email')){ 
                        echo '<i class="fa fa-envelope-o"></i> ';
                        echo $userres['email']; } ?></div>
                </div>
                <div class="bottom">
                    <?php if(can_show($userres, $permres, 'fb', 'show_fb')){ ?>
                    <a class="btn btn-primary btn-sm" href="<?php echo $userres['fb']; ?>">
                        <i class="fa fa-facebook"></i>
                    </a>
                    <?php } ?>
                    <?php if(can_show($userres, $permres, 'twitter', 'show_twitter')){ ?>
                    <a class="btn btn-primary btn-twitter btn-sm" href="<?php  echo $userres['twitter']; ?>">
                        <i class="fa fa-twitter"></i>
                    </a>
                    <?php } ?>
                    <?php if(can_show($userres, $permres, 'linkedin', 'show_linkedin')){ ?>
                    <a class="btn btn-primary btn-sm" href="<?php echo $userres['linkedin']; ?>">
                        <i class="fa fa-linkedin"></i>
                    </a>
                    <?php } ?>
                    <?php if(can_show($userres, $permres, 'blog', 'show_blog')){ ?>
                    <a class="btn btn-primary btn-sm" href="<?php echo $userres['blog']; ?>">
                        <i class="fa fa-rss"></i>
                    </a>
                    <?php } ?>
                    <?php if(can_show($userres, $permres, 'website', 'show_website')){ ?>
